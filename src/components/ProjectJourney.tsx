...
export function ProjectJourney({ projectId, stages, alerts }: { projectId: string; stages: Stage[]; alerts: JourneyAlert[] }) {
      ...
      <div className="flex items-start overflow-x-auto pb-1">
        {stages.map((s, i) => (
          <div key={s.key} className="flex items-center">
            <Link href={`/projects/${projectId}/${ROUTE_BY_STAGE[s.key]}`} className="flex flex-col items-center gap-1.5 w-[86px] shrink-0 group">
              <span
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium ${DOT[s.status]} group-hover:opacity-80`}
              >
                {s.status === "done" ? "✓" : i + 1}
              </span>
