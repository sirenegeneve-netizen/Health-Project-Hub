...
export default async function RunPage({ params }: { params: { id: string } }) {
  const initiative = await prisma.initiative.findUnique({ where: { id: params.id }, include: { interfaces: true, risks: true, trainings: true } });
  if (!initiative) notFound();

  const [backlogItems, kpis] = await Promise.all([
    prisma.backlogItem.findMany({ where: { initiativeId: params.id }, orderBy: { createdAt: "desc" } }),
    prisma.kpi.findMany({ where: { initiativeId: params.id }, orderBy: { createdAt: "desc" } }),
  ]);

  const blockingInterfaces = initiative.interfaces.filter((i) => i.isBlocking || i.status === "bloquant").length;
  const criticalOpenRisks = initiative.risks.filter((r) => ["forte", "critique"].includes(r.criticite) && !["maitrise", "cloture"].includes(r.status)).length;
  const totalUsers = initiative.trainings.reduce((s, t) => s + t.nbUsers, 0);
  const totalAutonomous = initiative.trainings.reduce((s, t) => s + (t.autonomyLevel >= 2 ? t.nbFormes : 0), 0);
  ...
  return (
    <div>
      ...
      <section className="mt-10">
        ...
        <BacklogForm initiativeId={params.id} />
