  ...
  return (
    <div>
      ...
      <section className="mt-8 mb-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <EditableField initiativeId={params.id} field="hypotheses" label="Hypothèses" placeholder="Ce qu'on suppose vrai au démarrage" initial={initiative.hypotheses || ""} />
          <EditableField initiativeId={params.id} field="contraintes" label="Contraintes" placeholder="Délais, budget, ressources, réglementation…" initial={initiative.contraintes || ""} />
        </div>
      </section>

      <section className="mt-10">
        <SectionTitle>Parties prenantes</SectionTitle>
        <StakeholderForm initiativeId={params.id} />
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href={`/initiatives/${params.id}/actors`} className="text-sm text-blue hover:underline">
            Ouvrir la matrice →
          </Link>
        </div>
      ...
      <section className="mt-10">
        ...
        <RiskForm initiativeId={params.id} actors={actors} establishments={initiative.establishments.map((e) => ({ id: e.establishmentId, name: e.establishment.name }))} />
        {risks.length > 0 ? (
      ...
      <section className="mt-10">
        ...
        <BudgetTargetsForm initiativeId={params.id} budgetInitialEur={initiative.budgetInitialEur} budgetReviseEur={initiative.budgetReviseEur} />
        {budget ? (
      ...
      <section className="mt-10 mb-4">
        ...
        {initiative.baselines.length > 0 ? (
          <div className="card">
            <ul className="text-sm space-y-1.5">
              {initiative.baselines.map((b) => (
