...
export default async function MeetingDetailPage({ params }: { params: { id: string; meetingId: string } }) {
  ...
  if (!meeting || meeting.initiativeId !== params.id) notFound();

  const previousMeeting = await prisma.meeting.findFirst({
    where: { initiativeId: params.id, date: { lt: meeting.date } },
    orderBy: { date: "desc" },
  });

  const since = previousMeeting?.date ?? new Date(0);

  const [allActions, allRisks, pendingDecisions, vigilancePoints, eventsSince, score, actors, establishmentLinks] = await Promise.all([
    ...
    prisma.decision.findMany({ where: { initiativeId: params.id, status: { not: "decision_prise" } } }),
    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <ActionForm initiativeId={params.id} meetingId={meeting.id} actors={actors} establishments={establishments} label="+ Action" />
          <RiskForm initiativeId={params.id} meetingId={meeting.id} actors={actors} establishments={establishments} label="+ Risque" />
          <DecisionForm initiativeId={params.id} meetingId={meeting.id} actors={actors} label="+ Décision" />
        </div>
      </div>
