...
export default async function CloturePage({ params }: { params: { id: string } }) {
  const initiative = await prisma.initiative.findUnique({ where: { id: params.id }, include: { budgetLines: true } });
  if (!initiative) notFound();

  await ensureStageCriteria(params.id, "cloture", initiative.type);

  const [criteria, openActions, openRisks] = await Promise.all([
    prisma.stageCriterion.findMany({ where: { initiativeId: params.id, stageKey: "cloture" }, orderBy: { order: "asc" } }),
    prisma.action.count({ where: { initiativeId: params.id, status: { notIn: ["termine", "abandonne"] } } }),
    prisma.risk.count({ where: { initiativeId: params.id, status: { notIn: ["maitrise", "cloture"] } } }),
  ]);
  ...
  return (
    <div>
      ...
      <p className="text-sm text-ink/50 mt-4">
        ...
        <Link href={`/initiatives/${params.id}/actions`} className="text-blue hover:underline">Déploiement</Link>{" "}
        et{" "}
        <Link href={`/initiatives/${params.id}/risks`} className="text-blue hover:underline">Risques</Link>.
      </p>
    </div>
  );
}
