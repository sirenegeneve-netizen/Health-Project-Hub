...
export default async function KickoffPage({ params }: { params: { id: string } }) {
  ...
  const [stakeholders, actors, meetings, criteria] = await Promise.all([
    ...
    prisma.actor.findMany({ where: { projectId: params.id } }),
    prisma.meeting.findMany({ where: { projectId: params.id }, include: { actions: true, decisions: true }, orderBy: { date: "desc" } }),
    prisma.stageCriterion.findMany({ where: { projectId: params.id, stageKey: "kickoff" }, orderBy: { order: "asc" } }),
  ]);
  ...
  return (
    <div>
      ...
      <div className="card mb-6">
        ...
        <Link href={`/projects/${params.id}/cadrage`} className="text-sm text-blue hover:underline mt-2 inline-block">
