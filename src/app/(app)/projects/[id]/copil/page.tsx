...
export default async function CopilPage({ params }: { params: { id: string } }) {
  ...
  const stages = getLifecycleStages(project.phase);
  ...
  const upcomingActions = project.actions
  ...
  const lastBaseline = project.baselines[project.baselines.length - 1];
  const firstBaseline = project.baselines[0];
  const dependencies = findSinglePointsOfFailure(project.raciEntries, new Map(project.actors.map((a) => [a.id, a.name])));

  const actorsByLoad = project.actors
    .map((a) => ({
      name: a.name,
      workload: computeActorWorkload(
        a,
        { actions: project.actions, risks: project.risks, interfaces: project.interfaces, deliverables: project.deliverables },
        project.raciEntries
      ).totalOwned,
    }))
  ...
  return (
    <div className="max-w-3xl mx-auto print:max-w-none">
      ...
      <div className="mb-6">
        ...
        <h1 className="font-display text-3xl text-ink mt-1">{project.name}</h1>
        <div className="text-sm text-ink/60 mt-1">
          {project.establishments.map((e) => e.establishment.name).join(", ") || "Établissement non renseigné"}
          {project.chefDeProjet ? ` · Chef de projet : ${project.chefDeProjet}` : ""}
        </div>
      </div>
      ...
      <Section n={7} title="Planning">
        <p className="text-sm text-ink/70">
          {project.targetDate ? (
            <>Date cible : <span className="font-medium text-ink">{new Date(project.targetDate).toLocaleDateString("fr-FR")}</span></>
          ) : (
