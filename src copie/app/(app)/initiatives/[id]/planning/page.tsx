...
export default async function PlanningPage({ params, searchParams }: { params: { id: string }; searchParams: { vue?: string } }) {
  ...
  if (actions.length === 0 && initiative.baselines.length <= 1) {
    return (
      <div>
        <InitiativeTabs initiativeId={params.id} />
        <h1 className="font-display text-2xl text-ink mb-4">Planning</h1>
        <ActionForm initiativeId={params.id} actors={actors} establishments={establishments} label="+ Ajouter une tâche" />
