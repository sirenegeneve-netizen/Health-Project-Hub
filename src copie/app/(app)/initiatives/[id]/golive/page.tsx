...
export default async function GoLivePage({ params }: { params: { id: string } }) {
  ...
  const unvalidatedInterfaces = initiative.interfaces.filter((i) => i.status !== "valide" && !(i.isBlocking || i.status === "bloquant"));
  const criticalAnomalies = initiative.anomalies.filter((a) => a.criticite === "critique" && !["corrigee", "validee"].includes(a.status));
  ...
  const lastGoNoGo = initiative.decisions.find((d) => d.subject.toLowerCase().includes("go/no go") || d.subject.toLowerCase().includes("go no go"));
    ...
    <div>
      ...
      {criticalAnomalies.length > 0 && (
        <p className="text-sm mb-4">
          <Link href={`/initiatives/${params.id}/validation`} className="text-blue hover:underline">
      ...
      <GoNoGoDecisionForm initiativeId={params.id} />
    </div>
  );
}
