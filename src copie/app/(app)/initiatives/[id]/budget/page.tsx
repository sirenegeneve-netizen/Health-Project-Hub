...
export default async function BudgetPage({ params }: { params: { id: string } }) {
  const initiative = await prisma.initiative.findUnique({ where: { id: params.id }, include: { budgetLines: { orderBy: { createdAt: "desc" } } } });
  if (!initiative) notFound();

  const summary = computeBudgetSummary(initiative.budgetInitialEur, initiative.budgetReviseEur, initiative.budgetLines);
  ...
  return (
    <div>
      ...
      {initiative.budgetLines.length > 0 ? (
        <BudgetLinesTable lines={initiative.budgetLines} />
      ) : (
