...
// > Initiative, §6 du document de conception). Partagé entre le script CLI
// (prisma/seed-workflow-stages.ts) et la route admin déclenchable depuis le
// navigateur (src/app/api/admin/seed-workflow-stages).

export type StageSeed = { key: string; label: string; legacyPhases?: string[] };

export const WORKFLOWS: Record<string, StageSeed[]> = {
  deploiement: [
